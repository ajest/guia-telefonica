...
export default function SurnamesTr ({ neighborhood, province }) {
  ...
  const trs = surnames.map((surname, key) =>
    <tr key={key}>
      ...
      {type === 'province' && <td>
        <a href={`https://www.abctelefonos.com/search?q=${surname}&l=${abcTelefonosZone}%2C+Argentina&t=persona&country=argentina`} target="_blank" rel="noreferrer">Link</a>
      </td>}
      <td>
      ...
      {type === 'province' && <td>
        <a href={`https://www.google.com.ar/maps/search/${surname},+${province.url}`} target="_blank" rel="noreferrer">Link</a>
      </td>}
    </tr>
  )
